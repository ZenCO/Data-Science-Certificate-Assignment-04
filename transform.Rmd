...
```{r}
flights %>% mutate(gain = dep_delay - arr_delay) %>% filter(dep_delay >=60 & gain>30 )
```
    
    1. Departed between midnight and 6am (inclusive)
    
```{r}
flights %>% filter(dep_time ==2400 | dep_time <=0600)
```
    
```{r}
flights %>% filter(between(dep_time,0100,0600))
```
